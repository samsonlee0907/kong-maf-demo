...
This repository demonstrates one agent through three operator-selectable paths:

- **Cloud Gateway mode**: `Browser -> Azure Kong -> Foundry Hosted Agent -> model runtime`
- **Local Kong mode**: `Browser -> local Kong -> FastAPI adapter -> MAF agent -> Foundry project endpoint`
- **Direct Preview mode**: `Browser -> local FastAPI adapter -> MAF agent -> Foundry project endpoint`
...
- now exposes explicit **Cloud Gateway**, **Local Kong**, and **Direct Preview** mode switches
...
These are rendered from `kong/kong.template.yaml` into `kong/kong.yaml` by the local startup script.
...
Use this when you want the full local gateway hop:

```text
Portal -> local Kong -> FastAPI adapter -> MAF agent
```

This is the mode to use when you want the same route split and buffering behavior locally before involving Azure resources.
...
|   |-- kong.template.yaml
|   |-- render-local-config.ps1
|   |-- render-local-config.sh
|   |-- start-local-kong.ps1
|   |-- start-local-kong.sh
|   +-- start-kong.sh
|-- tests/
|   |-- test_both.ps1
...
- **Local Kong** for `Portal -> local Kong -> FastAPI adapter`
- **Direct Preview** for `Portal -> local FastAPI adapter`
...
Set-Location .\kong
.\start-local-kong.ps1 -AgentPort 8080
```

If your FastAPI server is using `8088` instead of `8080`:

```powershell
Set-Location .\kong
.\start-local-kong.ps1 -AgentPort 8088
```
...
What `start-local-kong.ps1` does:

- renders `kong/kong.yaml` from `kong/kong.template.yaml`
- points Kong at `host.docker.internal:<your FastAPI port>`
- checks that Docker Desktop is actually ready
- fails fast if another local process already owns port `8000`
- starts the local Kong container on `127.0.0.1:8000`
...
The publishable repo should also stay free of:

- generated `kong/kong.yaml`
- local side projects or experiments outside the documented layout
- large runtime log files from `agent/`
...
- ensure the FastAPI server is running on the same port passed to `.\kong\start-local-kong.ps1 -AgentPort ...`
- regenerate local Kong if you change the FastAPI port

Local Kong returns `404`

- confirm `http://127.0.0.1:8000/health` returns the Kong health payload, not another local service
- if another process is already bound to `8000`, stop it or move that service before starting local Kong
- if Docker is not installed, Local Kong cannot run on that machine; use `Direct Preview` or `Cloud Gateway`
